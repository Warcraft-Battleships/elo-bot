...
handler = logging.basicConfig(filename='elobot.log',level=logging.INFO,format='%(asctime)s:%(levelname)s:%(name)s: %(message)s')
...
async def help(ctx, *command):
    if len(command) > 0:
        ...
        if command == "delete_account":
            ...
            await ctx.send(embed=embed)
        if command == "stats":
            add_text = "?stats <player>\n<player> is optional and must be provided as a discord mention\n\nDisplay yours or someone else's stats on the current ranked season" 
            embed = discord.Embed(title="Stats", description=add_text, color=0x00ffad)
            await ctx.send(embed=embed)
        if command == "allstats":
            add_text = "?allstats <player>\n<player> is optional and must be provided as a discord mention\n\nDisplay all of yours or someone else's stats for elo-eligible replays" 
...
async def elo(ctx):
    ...
    if row is not None:
        if row[1] == None:
            ...
            return
        elo = disp_elo(row[1],row[2])
        mmr = round(50*row[1])
        await ctx.channel.send("User *" + row[0] +"* have a current elo of " + str(elo) )#+" and an MMR of *" + str(mmr) + "*")
    else:
        await ctx.channel.send("User *" + ctx.author.name + "* doesn't have a bnet account registered, you might want to have a look at the add command ;)" )
...
async def allstats(ctx):
    ...
    for mention in ctx.message.mentions :
        ...
        query = "SELECT wc3_name,alias,games_played,K,D,A,dodosfound FROM `player` WHERE discord_id = " + str(ctx.message.mentions[0].id)
        break
    
    if not amention:
        player = ctx.author.nick
        query = "SELECT wc3_name,alias,games_played,K,D,A,dodosfound FROM `player` WHERE discord_id = " + str(ctx.author.id)
    cursor = my_db.cursor()
    cursor.execute(query)
    ...
    if result is not None :
        #update stats first
        query = "SELECT SUM(1),SUM(win),SUM(kills),SUM(deaths),SUM(assists),SUM(APM),SUM(staypercent),SUM(creepkills),SUM(bounty),SUM(bountyfeed),SUM(goldgathered),SUM(dodosfound),SUM(chatcounter),SUM(kickcounter) FROM crossfire_stats WHERE wc3_name = '" + result[0] + "'"
        cursor.execute(query)
        row = cursor.fetchone()
        if row is not None :
            total_games = row[0]
            if total_games == 0 or total_games == None:
                await ctx.channel.send("Player is registered, but there is no data about him ='( play and upload an eligible game to show your stats !")
                return
            total_win = row[1]
            ...
            stats_text = stats_text.format(total_games,mean_staypercent,win_percentage,KD,mean_assist,MGB,mean_APM)
    ...
    if len(players) % 2 == 0:
        ...
        for mention in ctx.message.mentions:      
            ...
            if row is not None:
                ...
                await ctx.send("player " + mention.display_name + " was not found, using fresh new player elo for that player")
...
#             T2.append([item[0] for item in team_b]) 
            score.append([trueenv.quality([[item[1] for item in team_a],[item[1] for item in team_b]]),[item[0] for item in team_a],[item[0] for item in team_b],[item[1] for item in team_a],[item[1] for item in team_b]])
...
async def maps(ctx):
    ...
    msg = ""
    while row is not None:
        msg = msg + "////wc3stats_key = " + str(row[0]) + "  ////filename = " + str(row[1]) + "  ////hash = "+ str(row[3]) + "\n"
        row = cursor.fetchone()
...
def is_this_valid(file_checksum,replay_hash,t0,t1,others,pcount,unregistered):
    ...
    if (len(t0) not in range(3,6) or len(t1) not in range(3,6)) or others or unregistered/pcount > 0.3:#unregistered/pcount > 0.99
...
def replay_parse(replay_response):
    ...
    try:
        ...
        sqlquery = sqlquery.format(wc3stats_id, gn, valid, timestamp, duration, season, map_filename, map_checksum,replay_hash)
    ...
    for i in range(len(replay_response['body']['data']['game']['players'])):
        ...
        sqlquery = "INSERT INTO crossfire_stats (wc3_name,game_id,win,elo_change,elo_confidence_change,kills,deaths,assists,APM,staypercent,creepkills,bounty,bountyfeed,goldgathered,lumbergathered,dodosfound,chatcounter,kickcounter,shiplist) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
        params = [wc3_name,wc3stats_id,win,elo_change[name.index(wc3_name)],elo_confidence_change[name.index(wc3_name)],kills,deaths,assists,APM,staypercent,creepkills,bounty,bountyfeed,goldgathered,lumbergathered,dodosfound,chatcounter,kickcounter,shiplist]
    ...
    discord_message = "Replay sent (map_id = " + str(map_checksum) + ") => https://wc3stats.com/games/" + str(replay_response['body']['id'])
...
async def new_season(ctx):
    ...
    if big_decision_admin_count <= 0:
        ...
        await ctx.channel.send("Admin <@!" + str(ctx.author.id) +"> is asking for a new ranked season to start ! \nat least " + str(big_decision_admin_count) + " other admin are needed to confirm this ! hurry ... winter is coming")
...
async def new_season_flush():
    ...
    while row is not None:
        ...
        new_elo_convergence = start_elo_convergence
        query = "UPDATE player SET elo = " + str(new_elo) + ",elo_convergence = " + str(new_elo_convergence) + " WHERE wc3_name = '" + row[0] + "'"
